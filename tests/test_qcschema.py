...
def test_gfn2xtb_energy():
    ...
    assert (
        approx(atomic_result.properties.scf_dipole_moment, abs=thr)
        == dipole_moment
    )
...
def test_gfn1xtb_gradient():
    ...
    gradient = np.array(
        [
            ...
            [
                -0.011856864082491841,
                -0.001160759424710484,
                -0.001479499047578632,
            ],
            ...
            [
                -0.000379213106866044,
                -0.002675726930398858,
                0.001107252098240491,
            ],
            [
                -0.007936554347068041,
                0.005513289713065560,
                -0.010832254028311825,
            ],
            [0.006084605665938956, 0.013967585988624595, -0.009310025918892868],
            [
                -0.003220049379416426,
                -0.003946107654179984,
                -0.003740489224738476,
            ],
            [0.006756157759172355, 0.000984515116819424, 0.007424736434524648],
            [
                -0.030710275643265804,
                -0.004788736649680724,
                0.009562034140682116,
            ],
            ...
            [
                0.011742825276516265,
                -0.002780169889933200,
                -0.001075047642530233,
            ],
            [
                -0.007336820690528053,
                -0.002159490005562796,
                -0.004872570579801525,
            ],
            [
                -0.000541853527432064,
                0.000671321722173119,
                -0.003239422092578492,
            ],
    ...
    assert atomic_result.success
    assert (
        approx(atomic_result.properties.return_energy, abs=thr)
        == -33.63768565903155
    )
    assert (
        approx(atomic_result.properties.scf_dipole_moment, abs=thr)
...
@pytest.mark.parametrize(
    "verbosity",
    [pytest.param("muted", id="muted flag"), pytest.param(0, id="muted value")],
)
def test_verbosity(verbosity):
    ...
    assert (
        atomic_result.return_result is not None
    )  # make sure the calculation was run
